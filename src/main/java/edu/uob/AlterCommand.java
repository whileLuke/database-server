...
public class AlterCommand extends DBCommand {
    ...
    public DBResponse query() throws IOException {
        ...
        if (commandType.equalsIgnoreCase("ADD")) {
            ...
            if (!success) {
                return DBResponse.error("Column '" + columnName + "' already exists in table '" + tableName + "'.");
            }
        }
        else if (commandType.equalsIgnoreCase("DROP")) {
            success = table.dropColumn(columnName);
            if (!success) {
                return DBResponse.error("Column '" + columnName + "' does not exist in table '" + tableName + "'.");
            }
        }
        else {
            return DBResponse.error("Unknown ALTER operation: " + commandType + ". Please use either ADD or DROP.");
        }

        // Save changes
        if (saveCurrentDB()) {
            return DBResponse.success("Table '" + tableName + "' altered successfully.");
        } else {
            return DBResponse.error("Failed to save altered table to the disk.");
        }
    }
}
