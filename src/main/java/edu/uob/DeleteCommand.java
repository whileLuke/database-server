...
public class DeleteCommand extends DBCommand {
    ...
    public DBResponse query() throws IOException {
        ...
        try {
            ...
            if (deletedRows > 0) {
                if (saveCurrentDB()) return DBResponse.success(deletedRows + " row(s) deleted.");
                else return DBResponse.error("Failed to save database after deletion.");
            } else return DBResponse.error("No rows matched the delete condition.");
        } catch (Exception e) {
